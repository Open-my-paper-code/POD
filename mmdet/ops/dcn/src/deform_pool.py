...
class ProjectionLIP(nn.Module):
    def __init__(self, channels):
        ...
        self.logit = nn.Sequential(
            OrderedDict((
                ('conv', GConv(channels, channels, 3, padding=1, bias=False, groups=4)),
...
class DeformRoIPoolingFunction(Function):
    ...
    def forward(ctx,
        ...
        ctx.out_size = (out_size, out_size)
...
class DeformRoIPooling(nn.Module):
    ...
    def __init__(self,
        ...
        self.part_size = self.out_size if part_size is None else part_size
    ...
    def forward(self, data, rois):
        ...
        if self.no_trans:
            ...
            x = deform_roi_pooling(data, rois, offset, self.spatial_scale,
                                   self.out_size[0], self.out_channels, True,
                                   self.group_size, self.part_size[0],
            ...
            roi_feats = deform_roi_pooling(data, rois, offset, self.spatial_scale,
                               self.out_size[0], self.out_channels,
                               self.no_trans, self.group_size,
                               self.part_size[0], self.sample_per_part,
                               self.trans_std)

            # roi_feats = self.lip(roi_feats)
